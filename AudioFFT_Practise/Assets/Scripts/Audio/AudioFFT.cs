...
public class AudioFFT : MonoBehaviour
{
    ...
    private void Start()
    {
        _FFT_cubes = new GameObject[spectrumDtatSize];
        ...
        {
            GameObject go = Instantiate(prb, transform);
            go.name = "FFT_Cube_" + i.ToString("000");
            this.transform.eulerAngles = new Vector3(0, -0.703125f * i, 0);
            go.transform.position += Vector3.forward * cycle_radius;
            _FFT_cubes[i] = go;
        }
    ...
    private void GetSpectrumAudioSource()
    {
        ...
        for (int i = 0; i < _FFT_cubes.Length; i++)
        {
            var v = yellowBarScale;
            v.y *= _samples[i] * _maxScale;
            _FFT_cubes[i].transform.localScale = v;
        }
    }
